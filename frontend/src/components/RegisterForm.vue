<template>
    <Form @submit="submitUser" :validation-schema="userFormSchema" class="m-4">
        <div class="mb-3">
            <label class="form-label" style="color:#666666">Tên tài khoản</label>
            <Field type="text" class="form-control" name="name" v-model="userLocal.name"/>
            <ErrorMessage name="name" class="text-danger" />
        </div>
        <div class="mb-3">
            <label class="form-label" style="color:#666666">Địa chỉ email</label>
            <Field type="email" class="form-control" name="email" v-model="userLocal.email" />
            <ErrorMessage name="email" class="text-danger" />
        </div>
        <div class="mb-4">
            <label class="form-label" style="color:#666666">Mật khẩu</label>
            <Field type="password" class="form-control" name="password" v-model="userLocal.password" />
            <div class="form-text">Đây là bảo mật.</div>
            <ErrorMessage name="password" class="text-danger" />

        </div>
        <div class="mb-4">
            <label class="form-label" style="color:#666666">Xác nhận mật khẩu</label>
        ...
        <button type="submit" class="btn text-light col-12" style="background-color: #e74c3c;">Đăng ký</button>
    </Form>
</template>
    ...
    export default {
        ...
        data() {
            const userFormSchema = yup.object().shape({
                name: yup
                    .string()
                    .required("Tên người dùng phải hợp lệ!")
                    .min(2, "Yêu cầu  tối thiểu 8 ký tự!"),
                email: yup
                    .string()
